...
// Design premium com tema dourado, modo escuro/claro, neon sutil
// ============================================================
const DashboardDITE = ({ user, onLogout }) => {
  ...
  const C = useMemo(() => ({
    ...
    neonGoldGlow: isDark ? '0 0 20px rgba(212,160,23,0.15), 0 0 40px rgba(212,160,23,0.08)' : '0 0 12px rgba(212,160,23,0.08)',
  ...
  const StatusBadge = ({ estado }) => {
    const config = {
      'APROVADO': { bg: C.successNeon, color: C.success, neon: isDark ? '0 0 8px rgba(74,222,128,0.3)' : 'none' },
      'REJEITADO': { bg: C.dangerNeon, color: C.danger, neon: isDark ? '0 0 8px rgba(248,113,113,0.3)' : 'none' },
      'EM_ANDAMENTO': { bg: C.warningNeon, color: C.warning, neon: isDark ? '0 0 8px rgba(251,191,36,0.3)' : 'none' },
      'FINALIZADO': { bg: `${C.textSoft}15`, color: C.textMuted, neon: 'none' },
      'PENDENTE_DITE': { bg: C.warningNeon, color: C.warning, neon: isDark ? '0 0 8px rgba(251,191,36,0.2)' : 'none' },
      'PENDENTE_DIRECAO': { bg: C.goldNeonSoft || 'rgba(212,160,23,0.15)', color: C.gold, neon: isDark ? '0 0 8px rgba(212,160,23,0.3)' : 'none' },
      'PENDENTE_ADMIN': { bg: C.infoNeon, color: C.info, neon: isDark ? '0 0 8px rgba(96,165,250,0.3)' : 'none' },
    };
  ...
  // Stat Card with Neon Edge
  const StatCard = ({ icon, label, value, subtitle, color, neonColor, isGold = false, delay = 0 }) => (
    <div className="ds-fade" style={{
      ...
      minHeight: 130, transition: 'all .3s ease',
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
          <div style={{
            ...
            boxShadow: isGold ? 'none' : `0 0 12px ${neonColor}`
          }}>{icon}</div>
      ...
      {!isGold && (
        <div style={{ position: 'relative', zIndex: 1, marginTop: 12 }}>
          <div style={{ height: 3, background: C.surfaceAlt, borderRadius: 2, overflow: 'hidden' }}>
            <div style={{
              height: '100%',
              width: `${totalGeral > 0 ? (parseInt(value) / totalGeral) * 100 : 0}%`,
  ...
  const TabButton = ({ label, count, active, onClick }) => (
    <button onClick={onClick} style={{
      ...
      boxShadow: active && isDark ? `0 0 12px ${C.neonGold}` : 'none'
    }}
    ...
    <div style={{
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @keyframes glow{0%,100%{filter:drop-shadow(0 0 2px ${C.gold}40)}50%{filter:drop-shadow(0 0 8px ${C.gold}60)}}
        @keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}
        @keyframes spin{to{transform:rotate(360deg)}}
        .ds-fade{animation:fadeIn .4s cubic-bezier(.4,0,.2,1) both}
        .ds-fade-scale{animation:fadeInScale .35s cubic-bezier(.4,0,.2,1) both}
        ...
        @media(max-width:768px){
          ...
          .ds-tabs{overflow-x:auto;flex-wrap:nowrap!important;gap:6!important;padding-bottom:4px;}
          .ds-tabs button{white-space:nowrap;flex-shrink:0;}
        }
        @media(max-width:480px){
          .ds-stats-grid{grid-template-columns:1fr!important;}
          .ds-stat-value{font-size:28px!important;}
        }
      `}</style>
      ...
      <aside className={`ds-sidebar ${mobileMenuOpen ? 'open' : ''}`} style={{
        ...
        <div style={{ padding: '28px 20px 0', position: 'relative' }}>
          <div style={{
            ...
            <div style={{
              ...
              opacity: 0.4, boxShadow: isDark ? `0 0 8px ${C.gold}40` : 'none'
            }} />
            <div style={{
              width: 44, height: 44, borderRadius: 14,
              background: `linear-gradient(135deg, ${C.gold}, ${C.goldDark})`,
              display: 'grid', placeItems: 'center', color: '#000', fontWeight: 900, fontSize: 18,
              boxShadow: isDark ? `0 4px 16px ${C.neonGold}, 0 0 24px ${C.neonGoldSoft}` : `0 4px 12px rgba(212,160,23,0.2)`,
        ...
        <div style={{ margin: '0 14px 16px', padding: 14, background: C.neonGoldSoft, borderRadius: 14, border: `1px solid ${C.gold}15`, display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{
            ...
            boxShadow: isDark ? `0 2px 8px ${C.neonGoldSoft}` : 'none'
          }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 10, color: C.goldMuted, fontWeight: 600, marginTop: 1, display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <nav style={{ padding: '0 10px', flex: 1 }}>
          ...
          {[
            ...
            return (
              <button key={item.id} onClick={() => { setAbaAtiva('painel'); setFiltroEstado(item.filter); setMobileMenuOpen(false); }} style={{
                ...
                boxShadow: isActive && isDark ? `inset 0 0 12px ${C.neonGoldSoft}` : 'none'
              }}
              ...
              >
                <span style={{ fontSize: 14, width: 22, textAlign: 'center', opacity: isActive ? 1 : 0.5, filter: isActive && isDark ? `drop-shadow(0 0 4px ${C.gold}40)` : 'none' }}>{item.icon}</span>
                <span style={{ flex: 1 }}>{item.label}</span>
                {isActive && <span style={{ width: 4, height: 4, borderRadius: '50%', background: C.gold, boxShadow: `0 0 4px ${C.gold}` }} />}
              </button>
            );
          })}

          <div style={{ borderTop: `1px solid ${C.border}`, margin: '16px 12px', position: 'relative' }}>
            <div style={{
          ...
          {[
            ...
            <button key={item.id} onClick={() => { setAbaAtiva(item.id); setMobileMenuOpen(false); }} style={{
              width: '100%', padding: '11px 14px', border: 'none',
              background: abaAtiva === item.id ? C.neonGoldSoft : 'transparent',
              color: abaAtiva === item.id ? C.gold : C.textMuted,
              fontWeight: abaAtiva === item.id ? 700 : 500, fontSize: 13, borderRadius: 10,
              cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 12,
              marginBottom: 2, fontFamily: 'inherit', transition: 'all .2s ease',
              boxShadow: abaAtiva === item.id && isDark ? `inset 0 0 12px ${C.neonGoldSoft}` : 'none'
            }}
            onMouseEnter={(e) => { if (abaAtiva !== item.id) { e.currentTarget.style.background = C.surfaceAlt; e.currentTarget.style.color = C.text; } }}
            onMouseLeave={(e) => { if (abaAtiva !== item.id) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = C.textMuted; } }}
            >
              <span style={{ fontSize: 14, width: 22, textAlign: 'center', opacity: abaAtiva === item.id ? 1 : 0.5 }}>{item.icon}</span>
              <span style={{ flex: 1 }}>{item.label}</span>
            </button>
          ))}
        </nav>
        ...
        <div style={{ padding: '0 14px' }}>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 16, filter: isDark ? 'none' : 'grayscale(.3)' }}>{isDark ? '☀' : '☾'}</span>
            ...
            <div style={{
              ...
              boxShadow: isDark ? `0 0 8px ${C.neonGold}` : 'none'
            }}>
              <div style={{
        ...
        <div style={{ padding: '10px 14px 20px' }}>
          ...
          onMouseEnter={(e) => { e.currentTarget.style.background = isDark ? 'rgba(248,113,113,0.12)' : 'rgba(220,38,38,0.08)'; e.currentTarget.style.borderColor = `${C.danger}30`; }}
          onMouseLeave={(e) => { e.currentTarget.style.background = isDark ? 'rgba(248,113,113,0.08)' : 'rgba(220,38,38,0.05)'; e.currentTarget.style.borderColor = isDark ? 'rgba(248,113,113,0.12)' : 'rgba(220,38,38,0.08)'; }}
          >
      ...
      <main className="ds-main" style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        <header className="ds-header" style={{
          ...
          transition: 'all .3s ease',
          boxShadow: scrollY > 10 ? C.shadow : 'none'
        }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2, display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{
                  ...
                  textShadow: isDark ? `0 0 8px ${C.neonGold}` : 'none'
                }}>{horaAtual}</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div ref={notifRef} style={{ position: 'relative' }}>
              ...
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = C.gold; e.currentTarget.style.boxShadow = isDark ? `0 0 8px ${C.neonGold}` : 'none'; }}
              ...
              >
                ...
                {notificacoesNaoLidas > 0 && (
                  <span style={{
                    ...
                    border: `2px solid ${isDark ? '#080808' : '#FEFCF3'}`,
                    boxShadow: `0 0 8px ${C.neonGold}`,
              ...
              {showNotifDropdown && (
                <div className="ds-fade-scale" style={{
                  ...
                  boxShadow: isDark ? `0 16px 48px rgba(0,0,0,.6), 0 0 16px ${C.neonGoldSoft}` : '0 16px 48px rgba(0,0,0,.12)',
                  ...
                  <div style={{
                    ...
                    {notificacoesNaoLidas > 0 && (
                      <span style={{
                        ...
                        boxShadow: `0 0 6px ${C.neonGold}`
                      }}>{notificacoesNaoLidas} novas</span>
                    )}
                  </div>
                  <div style={{ maxHeight: 360, overflowY: 'auto' }}>
                    {notificacoes.length === 0 ? (
                      ...
                      onMouseLeave={(e) => { e.currentTarget.style.background = n.lida ? 'transparent' : C.neonGoldSoft; }}
                      >
          ...
          {abaAtiva === 'painel' && (
            <div className="ds-fade" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <div className="ds-stats-grid" style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(4, 1fr)' }}>
                <StatCard icon="◷" label="Pendentes" value={totalPendentes} subtitle="Aguardando análise" color={C.gold} neonColor={C.neonGold} delay={0} />
                <StatCard icon="✓" label="Aprovados" value={totalAprovados} subtitle="Autorizados" color={C.success} neonColor={C.successNeon} delay={60} />
                <StatCard icon="✕" label="Rejeitados" value={totalRejeitados} subtitle="Negados" color={C.danger} neonColor={C.dangerNeon} delay={120} />
                <StatCard icon="◈" label="Total Geral" value={totalGeral} subtitle="Pedidos no sistema" color="#000" neonColor={C.neonGold} isGold delay={180} />
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                {[
                  ...
                  { label: 'Finalizados', value: stats.finalizados || pedidos.filter(p => p.estado === 'FINALIZADO').length, color: C.textMuted, neon: `${C.textSoft}20` },
                  ...
                  onMouseEnter={(e) => { e.currentTarget.style.boxShadow = isDark ? `0 4px 20px rgba(0,0,0,.5), 0 0 12px ${item.neon}` : '0 4px 20px rgba(0,0,0,.08)'; }}
              ...
              <div className="ds-toolbar" style={{
                ...
                <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
                  ...
                  <input type="text" placeholder="Buscar estudante, curso ou ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{
                    ...
                    fontFamily: 'inherit', fontSize: 13, outline: 'none', transition: 'all .2s ease'
                  }}
                  onFocus={(e) => { e.currentTarget.style.borderColor = C.gold; e.currentTarget.style.boxShadow = isDark ? `0 0 12px ${C.neonGold}` : '0 0 0 3px rgba(212,160,23,0.1)'; }}
                ...
                <select value={filtroEstado} onChange={(e) => setFiltroEstado(e.target.value)} style={{
                  ...
                  borderRadius: 10, color: C.text, fontFamily: 'inherit', fontSize: 12,
              ...
              <div className="ds-tabs" style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                ...
                <TabButton label="Finalizados" active={filtroEstado === 'FINALIZADO'} onClick={() => setFiltroEstado('FINALIZADO')} />
              </div>
              ...
              {loading ? (
                <div style={{ textAlign: 'center', padding: 80 }}>
                  <div style={{
                    ...
                    borderTopColor: C.gold, borderRadius: '50%', margin: '0 auto 16px',
                    animation: 'spin .8s linear infinite',
                    boxShadow: isDark ? `0 0 12px ${C.neonGold}` : 'none'
                  }} />
                  <div style={{ color: C.textMuted, fontSize: 12, fontWeight: 500 }}>Carregando pedidos...</div>
                </div>
              ) : pedidosFiltrados.length === 0 ? (
                <div style={{
                  textAlign: 'center', padding: 80, color: C.textMuted,
                  border: `1px dashed ${C.borderStrong}`, borderRadius: 16,
                  background: C.surface
                }}>
                  <div style={{ fontSize: 48, marginBottom: 16, opacity: .15, color: C.gold, textShadow: isDark ? `0 0 20px ${C.neonGold}` : 'none' }}>◈</div>
                ...
                <div className="ds-table-wrap" style={{
                  background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16,
                  ...
                  <table className="ds-table" style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                    <thead>
                      <tr style={{ borderBottom: `2px solid ${C.border}` }}>
                        {['ID', 'ESTUDANTE', 'CURSO / CLASSE', 'TIPO', 'DATA SAÍDA', 'STATUS', 'AÇÕES'].map(h => (
                          <th key={h} style={{
                            padding: '14px 16px', textAlign: 'left', fontWeight: 800,
                    ...
                    <tbody>
                      {pedidosFiltrados.map((pedido, idx) => {
                        const isHovered = hoveredRow === pedido.id;
                        return (
                          <tr key={pedido.id} style={{
                            borderBottom: `1px solid ${C.border}`,
                            background: isHovered ? C.neonGoldSoft : (idx % 2 === 0 ? 'transparent' : C.surfaceAlt + '40'),
                            transition: 'background .15s ease'
                          }}
                          onMouseEnter={() => setHoveredRow(pedido.id)}
                          onMouseLeave={() => setHoveredRow(null)}
                          >
                            <td style={{ padding: '14px 16px', fontWeight: 800, color: C.gold }}>
                              <span style={{
                                textShadow: isDark ? `0 0 6px ${C.neonGold}` : 'none'
                              }}>#{pedido.id}</span>
                            </td>
                              ...
                              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                ...
                                {pedido.acoes_disponiveis?.includes('aprovar') && (
                                  <button onClick={() => aprovarPedido(pedido.id)} style={{
                                    ...
                                    boxShadow: isDark ? `0 0 8px ${C.successNeon}` : 'none',
                                    transition: 'all .15s'
              ...
              {relatorios.length === 0 ? (
                ...
                <div style={{ display: 'grid', gap: 12 }}>
                  {relatorios.map((r, idx) => (
                    <div key={r.id} className="ds-fade" style={{
                      background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14,
                      padding: 20, boxShadow: C.shadow, display: 'flex', justifyContent: 'space-between',
                      alignItems: 'center', cursor: 'pointer', transition: 'all .2s ease',
                      flexWrap: 'wrap', gap: 12,
                      animationDelay: `${idx * 50}ms`, animationFillMode: 'both'
                    }}
                    onClick={() => baixarRelatorio(r.id)}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.boxShadow = isDark ? `0 8px 40px rgba(0,0,0,.5), 0 0 16px ${C.neonGoldSoft}` : '0 8px 40px rgba(0,0,0,.08)';
                    ...
                    >
                      ...
                      <div style={{
                        padding: '8px 16px', background: C.neonGoldSoft, border: `1px solid ${C.gold}20`,
                        borderRadius: 8, color: C.gold, fontSize: 11, fontWeight: 700,
                        boxShadow: isDark ? `0 0 8px ${C.neonGoldSoft}` : 'none'
                      }}>BAIXAR CSV →</div>
                    </div>
                  ))}
                </div>
      ...
      {modalDetalhes && (
        <div onClick={() => setModalDetalhes(null)} style={{
          ...
          <div onClick={e => e.stopPropagation()} className="ds-fade-scale" style={{
            ...
            boxShadow: isDark ? `0 24px 80px rgba(0,0,0,.5), 0 0 24px ${C.neonGoldSoft}` : '0 24px 80px rgba(0,0,0,.2)',
            ...
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 0 }}>
              {[
                ...
                <div key={i} style={{
                  ...
                  padding: '14px 0', borderBottom: i < 6 ? `1px solid ${C.border}` : 'none'
                }}>
            ...
            <div style={{
              ...
              {modalDetalhes.acoes_disponiveis?.includes('aprovar') && (
                <button onClick={() => { aprovarPedido(modalDetalhes.id); setModalDetalhes(null); }} style={{
                  ...
                  boxShadow: isDark ? `0 0 12px ${C.successNeon}` : 'none'
                }}>Aprovar</button>
              )}
              {modalDetalhes.acoes_disponiveis?.includes('rejeitar') && (
                <button onClick={() => { rejeitarPedido(modalDetalhes.id); setModalDetalhes(null); }} style={{
                  ...
                  fontWeight: 700, fontSize: 12, fontFamily: 'inherit',
                  boxShadow: isDark ? `0 0 12px ${C.dangerNeon}` : 'none'
                }}>Rejeitar</button>
              )}
              {modalDetalhes.estado === 'PENDENTE_DIRECAO' && (
                <button onClick={() => { handleEncaminhar(modalDetalhes.id); setModalDetalhes(null); }} style={{
                  ...
                  fontWeight: 700, fontSize: 12, fontFamily: 'inherit',
                  boxShadow: isDark ? `0 0 12px ${C.neonGoldSoft}` : 'none'
                }}>Encaminhar</button>
              )}
      ...
      {modalGerarRel && (
        <div onClick={() => setModalGerarRel(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} className="ds-fade-scale" style={{
            ...
            <div style={{
              fontSize: 18, fontWeight: 800, color: C.text, marginBottom: 6, letterSpacing: -.3,
              textShadow: isDark ? `0 0 12px ${C.neonGold}` : 'none'
            }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={gerarRelatorio} disabled={gerando} style={{
                ...
                fontWeight: 700, fontSize: 12, fontFamily: 'inherit',
                boxShadow: isDark ? `0 4px 16px ${C.neonGold}` : '0 4px 12px rgba(212,160,23,0.2)',
                opacity: gerando ? 0.6 : 1, transition: 'all .2s ease'
