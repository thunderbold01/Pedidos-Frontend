// src/pages/DashboardDITE.jsx - VERSÃO CORRIGIDA
...
const DashboardDITE = ({ user, onLogout }) => {
  ...
  const carregarDados = async () => {
    ...
    try {
      ...
      if (filtroData) {
        // Converter a data para o formato que o backend espera (YYYY-MM-DD)
        const dataFormatada = new Date(filtroData).toISOString().split('T')[0];
        params.append('data_inicio', dataFormatada);
        params.append('data_fim', dataFormatada);
      }
  ...
  // Filtro combinado: estado + data + busca
    ...
    <div style={styles.container}>
        ...
        <ul style={styles.navLinks}>
          {menuItems.map(item => (
            <li key={item.id} style={styles.navItem}>
              <a href="#" onClick={(e) => { e.preventDefault(); setFiltroEstado(item.id); setAba('pedidos'); setSidebarOpen(false); }} style={{ ...styles.navLink, background: filtroEstado === item.id ? 'linear-gradient(135deg, #2563EB, #1D4ED8)' : 'transparent', color: filtroEstado === item.id ? '#fff' : '#64748B', boxShadow: filtroEstado === item.id ? '0 4px 12px rgba(37, 99, 235, 0.25)' : 'none' }}>
                <i className={item.icon} style={{ width: 20 }}></i><span>{item.label}</span>
                {item.count > 0 && <span style={{ marginLeft: 'auto', background: filtroEstado === item.id ? 'rgba(255,255,255,0.2)' : '#FEE2E2', color: filtroEstado === item.id ? '#fff' : '#991B1B', padding: '2px 8px', borderRadius: 10, fontSize: 11, fontWeight: 700 }}>{item.count}</span>}
              </a>
            </li>
          ))}
          <li><hr style={styles.divider} /></li>
          <li style={styles.navItem}><a href="#" onClick={(e) => { e.preventDefault(); setAba('coletivas'); setSidebarOpen(false); }} style={styles.navLink}><i className="fas fa-users"></i><span>Saídas Coletivas</span>{coletivas.length > 0 && <span style={styles.badge}>{coletivas.length}</span>}</a></li>
          <li style={styles.navItem}><a href="#" onClick={(e) => { e.preventDefault(); setAba('alertas'); setSidebarOpen(false); }} style={styles.navLink}><i className="fas fa-shield-alt"></i><span>Alertas Atraso</span>{alertasAtraso.length > 0 && <span style={{...styles.badge, background: '#DC2626', color: '#fff' }}>!</span>}</a></li>
          <li style={styles.navItem}><a href="#" onClick={(e) => { e.preventDefault(); setAba('relatorios'); setSidebarOpen(false); }} style={styles.navLink}><i className="fas fa-chart-line"></i><span>Relatórios</span>{relatoriosSeguranca.length > 0 && <span style={styles.badge}>{relatoriosSeguranca.length}</span>}</a></li>
          <li style={styles.navItem}><a href="#" onClick={(e) => { e.preventDefault(); setShowColetiva(true); setSidebarOpen(false); }} style={{...styles.navLink, color: '#059669' }}><i className="fas fa-plus-circle"></i><span>Nova Coletiva</span></a></li>
        </ul>
        <div style={styles.userCard}>
          <div style={styles.userAvatar}><img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user?.nome || user?.username || 'User')}&background=2563EB&color=fff`} alt="Avatar" style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover' }} /></div>
          <div style={styles.userInfo}><h4>{user?.nome || user?.username}</h4><p>DITE - Tecnologia</p></div>
      ...
      <div className="main-wrapper" style={styles.mainWrapper}>
        <header style={styles.topHeader}>
          <button className="menu-toggle" onClick={() => setSidebarOpen(!sidebarOpen)} style={styles.menuToggle}><i className="fas fa-bars"></i></button>
          <div style={styles.headerTitle}><h1>Visão Geral</h1><span>Bem-vindo de volta, aqui está o resumo de hoje.</span></div>
          <div style={styles.headerActions}>
            <button style={styles.iconBtn} onClick={carregarDados}><i className="fas fa-sync-alt"></i></button>
            <button style={{...styles.iconBtn, position: 'relative'}} onClick={() => setShowNotificacoes(!showNotificacoes)}><i className="fas fa-bell"></i>{notificacoesNaoLidas > 0 && <div style={styles.notificationDot}></div>}</button>
            <button style={styles.iconBtn} onClick={onLogout}><i className="fas fa-sign-out-alt"></i></button>
          </div>
        </header>
        ...
        <div style={styles.contentScroll}>
          ...
          <div style={styles.statsGrid}>
            <div style={styles.statCard}><div style={{...styles.statIcon, background: '#EFF6FF', color: '#2563EB' }}><i className="fas fa-clipboard-list"></i></div><div style={styles.statInfo}><h3>{stats.meus_pedidos_pendentes || 0}</h3><p>Pedidos Pendentes</p></div></div>
            <div style={styles.statCard}><div style={{...styles.statIcon, background: '#DCFCE7', color: '#166534' }}><i className="fas fa-check-circle"></i></div><div style={styles.statInfo}><h3>{stats.pedidos_aprovados || 0}</h3><p>Aprovados (Mês)</p></div></div>
            <div style={styles.statCard}><div style={{...styles.statIcon, background: '#FEF9C3', color: '#854D0E' }}><i className="fas fa-clock"></i></div><div style={styles.statInfo}><h3>{alertasAtraso.length}</h3><p>Alertas de Atraso</p></div></div>
          </div>
          ...
          <div style={styles.filterBar}>
            <div style={styles.filterGroup}>
              ...
              <div style={styles.filterButtons}>
                <button onClick={() => setFiltroEstado('PENDENTE_DITE')} style={{...styles.filterButton, background: filtroEstado === 'PENDENTE_DITE' ? '#2563EB' : '#F1F5F9', color: filtroEstado === 'PENDENTE_DITE' ? '#fff' : '#64748B'}}>Pendentes ({stats.meus_pedidos_pendentes || 0})</button>
                <button onClick={() => setFiltroEstado('APROVADO')} style={{...styles.filterButton, background: filtroEstado === 'APROVADO' ? '#2563EB' : '#F1F5F9', color: filtroEstado === 'APROVADO' ? '#fff' : '#64748B'}}>Aprovados ({stats.pedidos_aprovados || 0})</button>
                <button onClick={() => setFiltroEstado('REJEITADO')} style={{...styles.filterButton, background: filtroEstado === 'REJEITADO' ? '#2563EB' : '#F1F5F9', color: filtroEstado === 'REJEITADO' ? '#fff' : '#64748B'}}>Rejeitados ({stats.pedidos_rejeitados || 0})</button>
              </div>
            </div>
            <div style={styles.filterGroup}>
              <label style={styles.filterLabel}>Data de Saída:</label>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <input type="date" value={filtroData} onChange={(e) => setFiltroData(e.target.value)} style={styles.dateInput} />
                {filtroData && <button onClick={() => setFiltroData('')} style={styles.clearFilterBtn}><i className="fas fa-times"></i> Limpar</button>}
              </div>
            </div>
            <div style={styles.filterGroup}>
              ...
              <div style={styles.searchWrapper}>
                ...
                <input type="text" placeholder="Estudante ou ID..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={styles.searchInput} />
              </div>
            </div>
          </div>
          ...
          <div style={styles.tabsContainer}>
            <button onClick={() => setAba('pedidos')} style={{...styles.tab, background: aba === 'pedidos' ? '#fff' : 'transparent', borderBottom: aba === 'pedidos' ? '2px solid #2563EB' : 'none', color: aba === 'pedidos' ? '#2563EB' : '#64748B' }}><i className="fas fa-clipboard-list"></i> Pedidos ({pedidosFiltrados.length})</button>
            <button onClick={() => setAba('coletivas')} style={{...styles.tab, background: aba === 'coletivas' ? '#fff' : 'transparent', borderBottom: aba === 'coletivas' ? '2px solid #2563EB' : 'none', color: aba === 'coletivas' ? '#2563EB' : '#64748B' }}><i className="fas fa-users"></i> Coletivas ({coletivas.length})</button>
            <button onClick={() => setAba('alertas')} style={{...styles.tab, background: aba === 'alertas' ? '#fff' : 'transparent', borderBottom: aba === 'alertas' ? '2px solid #2563EB' : 'none', color: aba === 'alertas' ? '#2563EB' : '#64748B' }}><i className="fas fa-shield-alt"></i> Alertas ({alertasAtraso.length})</button>
            <button onClick={() => setAba('relatorios')} style={{...styles.tab, background: aba === 'relatorios' ? '#fff' : 'transparent', borderBottom: aba === 'relatorios' ? '2px solid #2563EB' : 'none', color: aba === 'relatorios' ? '#2563EB' : '#64748B' }}><i className="fas fa-chart-line"></i> Relatórios ({relatoriosSeguranca.length})</button>
          </div>
          ...
          <div style={styles.dataCard}>
            {aba === 'pedidos' && (
              <>
                <div style={styles.cardHeader}><h3 style={{ fontSize: 18, fontWeight: 600 }}>Pedidos</h3><div style={styles.resultsInfo}>Total: {pedidosFiltrados.length} pedidos</div></div>
                {loading ? <div style={styles.loading}>Carregando...</div> : pedidosFiltrados.length === 0 ? <div style={styles.emptyState}><i className="fas fa-inbox" style={{ fontSize: 48, color: '#94A3B8', marginBottom: 16 }}></i><p>Nenhum pedido encontrado</p></div> : (
                  <div style={{ overflowX: 'auto' }}>
                    <table style={styles.modernTable}>
                      <thead><tr><th>ID</th><th>Estudante</th><th>Tipo</th><th>Data/Hora</th><th>Status</th><th style={{ textAlign: 'center' }}>Ações</th></tr></thead>
                      <tbody>
                        {pedidosFiltrados.map(p => (
                          <tr key={p.id}>
                            <td style={{ fontWeight: 600, color: '#2563EB' }}>#{p.id}</td>
                            <td><div style={styles.studentCell}><img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(p.estudante_nome || 'User')}&background=random`} alt="" style={styles.studentImg} /><div><span style={styles.studentName}>{p.estudante_nome}</span><span style={styles.studentCourse}>{p.estudante_curso || '-'} / {p.estudante_classe || '-'}</span></div></div></td>
                            ...
                            <td style={{ textAlign: 'center' }}>
                              <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
                                ...
                                {p.acoes_disponiveis?.includes('aprovar') && <button onClick={() => abrirModalAprovacao(p.id)} style={styles.btnPrimary}><i className="fas fa-check"></i> Analisar</button>}
                                {p.acoes_disponiveis?.includes('rejeitar') && <button onClick={() => handleAcao(p.id, 'rejeitar')} style={styles.btnDanger}><i className="fas fa-times"></i> Rejeitar</button>}
                                {p.estado === 'PENDENTE_DITE' && <button onClick={() => setModalEncaminhar(p.id)} style={styles.btnWarning}><i className="fas fa-share"></i> Encaminhar</button>}
                              </div>
                            </td>
                          </tr>
                        ))}
            ...
            {aba === 'coletivas' && (
              <>
                <div style={styles.cardHeader}><h3 style={{ fontSize: 18, fontWeight: 600 }}>Saídas Coletivas</h3><button onClick={() => setShowColetiva(true)} style={styles.btnPrimary}><i className="fas fa-plus"></i> Nova Coletiva</button></div>
                {coletivas.length === 0 ? <div style={styles.emptyState}>Nenhuma saída coletiva criada</div> : coletivas.map(c => (
                  <div key={c.id} style={styles.coletivaCard}>
                    <div style={styles.coletivaHeader}><strong>{c.titulo}</strong><span style={{...styles.coletivaBadge, background: c.encerrada ? '#F1F5F9' : '#DCFCE7', color: c.encerrada ? '#64748B' : '#166534' }}>{c.encerrada ? 'Encerrada' : 'Ativa'}</span></div>
                    <div style={styles.coletivaInfo}><span>📅 Saída: {c.data_saida?.split('T')[0]}</span><span>🔙 Volta: {c.data_volta?.split('T')[0]}</span><span>👤 Criador: {c.criador_nome || c.criador}</span></div>
                    <div style={styles.progressBar}><div style={{ width: `${((c.total_aceitos || 0) / (c.total_convidados || 1)) * 100}%`, ...styles.progressFill }} /></div>
                    <div style={styles.coletivaStats}><span><strong>{c.total_convidados || 0}</strong> Convidados</span><span><strong style={{ color: '#166534' }}>{c.total_aceitos || 0}</strong> Aceitaram</span><span><strong style={{ color: '#991B1B' }}>{c.total_recusados || 0}</strong> Recusaram</span><span><strong>{c.total_pendentes || 0}</strong> Pendentes</span></div>
                  </div>
                ))}
              </>
            )}
            ...
            {aba === 'alertas' && (
              <>
                <div style={styles.cardHeader}><h3 style={{ fontSize: 18, fontWeight: 600 }}>Alertas de Atraso</h3></div>
                {alertasAtraso.length === 0 ? <div style={styles.emptyState}>Nenhum alerta de atraso</div> : alertasAtraso.map(a => (
                  <div key={a.id} onClick={marcarAlertaLido} style={{...styles.alertaCard, cursor: 'pointer'}}><div><strong>{a.estudante_nome}</strong> - Atraso de <strong>{a.tempo_atraso} minutos</strong></div><div style={{ fontSize: 12, color: '#64748B' }}>Enviado em: {a.enviado_em}</div>{!a.lido && <span style={styles.alertaBadge}>🔴 Não lido</span>}</div>
                ))}
              </>
            )}
            ...
            {aba === 'relatorios' && (
              <>
                <div style={styles.cardHeader}><h3 style={{ fontSize: 18, fontWeight: 600 }}>Relatórios de Segurança</h3></div>
                {relatoriosSeguranca.length === 0 ? <div style={styles.emptyState}>Nenhum relatório recebido</div> : relatoriosSeguranca.map(r => (
                  <div key={r.id} onClick={() => baixarRelatorio(r.id)} style={styles.relatorioCard}><div><strong>{r.titulo}</strong></div><div style={{ fontSize: 12, color: '#64748B' }}>Enviado por: {r.criado_por} em {r.created_at}</div><div style={{ fontSize: 11, color: '#2563EB' }}>📥 Clique para baixar</div></div>
                ))}
              </>
            )}
          </div>
      ...
      <div className={`modal-overlay ${modalAprovacao ? 'open' : ''}`} onClick={() => setModalAprovacao(null)}>
        <div className="modal-content" onClick={e => e.stopPropagation()}>
          <div style={styles.modalHeader}><h2>Detalhes da Aprovação</h2><p style={{ color: '#64748B', fontSize: 14 }}>Confirme os horários para o estudante</p><button onClick={() => setModalAprovacao(null)} style={styles.modalClose}>✕</button></div>
          <div style={styles.modalBody}>
            <div style={styles.formRow}><div style={styles.inputGroup}><label>Data de Saída</label><input type="date" value={dadosAprovacao.data_saida} onChange={e => setDadosAprovacao({...dadosAprovacao, data_saida: e.target.value})} style={styles.inputField} min={hoje} /></div><div style={styles.inputGroup}><label>Hora</label><input type="time" value={dadosAprovacao.hora_saida} onChange={e => setDadosAprovacao({...dadosAprovacao, hora_saida: e.target.value})} style={styles.inputField} /></div></div>
            <div style={styles.formRow}><div style={styles.inputGroup}><label>Previsão de Retorno</label><input type="date" value={dadosAprovacao.data_volta} onChange={e => setDadosAprovacao({...dadosAprovacao, data_volta: e.target.value})} style={styles.inputField} min={dadosAprovacao.data_saida} /></div><div style={styles.inputGroup}><label>Hora</label><input type="time" value={dadosAprovacao.hora_volta} onChange={e => setDadosAprovacao({...dadosAprovacao, hora_volta: e.target.value})} style={styles.inputField} /></div></div>
            <div style={styles.modalFooter}><button onClick={() => setModalAprovacao(null)} style={styles.btnCancel}>Cancelar</button><button onClick={confirmarAprovacao} style={styles.btnConfirm}>Confirmar Saída</button></div>
          </div>
        </div>
      </div>
      ...
      <div className={`modal-overlay ${modalEncaminhar ? 'open' : ''}`} onClick={() => setModalEncaminhar(null)}>
        <div className="modal-content" onClick={e => e.stopPropagation()}>
          <div style={styles.modalHeader}><h2>Encaminhar Pedido</h2><button onClick={() => setModalEncaminhar(null)} style={styles.modalClose}>✕</button></div>
          <div style={styles.modalBody}>
            <p style={{ marginBottom: 20 }}>Selecione o destino:</p>
            <button onClick={() => handleEncaminhar(modalEncaminhar, 'DIRECAO')} style={styles.destinoBtn}><span style={{ fontSize: 28 }}>👨‍💼</span><div><strong>Direção</strong><br /><small>Análise final</small></div></button>
            <button onClick={() => handleEncaminhar(modalEncaminhar, 'ADMINISTRACAO')} style={{...styles.destinoBtn, marginTop: 12}}><span style={{ fontSize: 28 }}>🏛️</span><div><strong>Administração</strong><br /><small>Recursos e documentos</small></div></button>
          </div>
        </div>
      </div>
      ...
      <div className={`modal-overlay ${showColetiva ? 'open' : ''}`} onClick={() => setShowColetiva(false)}>
        <div className="modal-content" onClick={e => e.stopPropagation()}>
          <div style={styles.modalHeader}><h2>👥 Nova Saída Coletiva</h2><button onClick={() => setShowColetiva(false)} style={styles.modalClose}>✕</button></div>
          <div style={styles.modalBody}>
            ...
            <form onSubmit={criarColetiva}>
              ...
              <div style={styles.formRow}><input type="datetime-local" placeholder="Data Saída" value={formColetiva.data_saida} onChange={e => setFormColetiva({...formColetiva, data_saida: e.target.value})} style={styles.inputField} required /><input type="datetime-local" placeholder="Data Volta" value={formColetiva.data_volta} onChange={e => setFormColetiva({...formColetiva, data_volta: e.target.value})} style={styles.inputField} required /></div>
              ...
              <div style={styles.modalFooter}><button type="button" onClick={() => setShowColetiva(false)} style={styles.btnCancel}>Cancelar</button><button type="submit" disabled={loadingColetiva} style={styles.btnConfirm}>{loadingColetiva ? 'Criando...' : 'Criar'}</button></div>
            </form>
          </div>
        </div>
      </div>
      ...
      {showNotificacoes && (
        <>
          ...
          <div style={styles.notifPanel}>
            <div style={styles.notifHeader}><h3>🔔 Notificações</h3><button onClick={() => setShowNotificacoes(false)} style={styles.notifClose}>✕</button></div>
            <div style={styles.notifList}>{notificacoes.length === 0 ? <div style={styles.notifEmpty}>Nenhuma notificação</div> : notificacoes.map(n => (<div key={n.id} onClick={() => { marcarNotificacaoLida(n.id); if (n.pedido_id) { navigate(`/pedido/${n.pedido_id}`); setShowNotificacoes(false); } }} style={{...styles.notifItem, background: n.lida ? '#fff' : '#FEFCE8'}}><div style={styles.notifMessage}>{n.mensagem}</div><div style={styles.notifDate}>{n.data}</div></div>))}</div>
          </div>
        </>
      )}
    </div>
...
const styles = {
  ...
  sidebar: { width: 280, background: '#FFFFFF', height: '100vh', padding: '24px', display: 'flex', flexDirection: 'column', borderRight: '1px solid #E2E8F0', zIndex: 20, transition: 'left 0.3s ease', position: 'sticky', top: 0, '@media (max-width: 768px)': { position: 'fixed', left: '-280px' } },
  ...
  userInfo: { flex: 1 },
  overlay: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', zIndex: 999, display: 'none', '@media (max-width: 768px)': { display: 'block' } },
  ...
  statCard: { background: '#fff', padding: 24, borderRadius: 20, border: '1px solid #E2E8F0', boxShadow: '0 1px 2px 0 rgb(0 0 0 / 0.05)', display: 'flex', alignItems: 'center', gap: 20, transition: 'transform 0.2s' },
  ...
  clearFilterBtn: { padding: '8px 12px', background: '#FEE2E2', border: 'none', borderRadius: 8, cursor: 'pointer', color: '#DC2626', fontSize: 12 },
  ...
  alertaCard: { background: '#FEF2F2', borderLeft: '4px solid #DC2626', padding: 12, marginBottom: 10, borderRadius: 8 },
  ...
  modalHeader: { padding: 20, borderBottom: '1px solid #E2E8F0' },
  modalClose: { float: 'right', width: 32, height: 32, border: '1px solid #E2E8F0', background: '#fff', borderRadius: 8, cursor: 'pointer' },
